...
@interface ShieldPostVC ()<UITableViewDataSource, UITableViewDelegate>
{
...
- (IBAction)actionSubmit:(id)sender{
    NSDictionary *parameter = @{@"token": TOKEN,
                                ...
                                @"tid": _dictPost[@"tid"],
                                @"pid": _dictPost[@"pid"],
                                @"banned": _isShield ? @"0" : @"1",
                                @"reason": _textView.text,
                                @"sendreasonpm": @"1"};
    [self.view showHUDActivityView:@"正在提交..." shade:NO];
    [EKHttpUtil mHttpWithUrl:kManageURL parameter:parameter response:^(BKNetworkModel *model, NSString *netErr) {
        [self.view removeHUDActivity];
                ...
                if (model.status) {
                    if (_isFromThread) {//如果是来自帖子内容页，返回之前刷新下帖子内容页的数据
                        ...
                        dispatch_async(queue, ^{
                            ...
                            [[NSNotificationCenter defaultCenter] postNotificationName:kUpdataRevertNotification object:_dictPost[@"pid"]];
                        });
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.row) {
        case 0:
        {
            cell.textLabel.text = _isShield ? @"解除屏蔽：" : @"將要屏蔽";
        ...
        case 1:
        {
            ...
            //cell.detailTextLabel.text = @"違規內容";
...
+(void)push:(UIViewController *)vc detailModel:(ThreadsDetailModel *)detailModel dataModel:(InvitationDataModel *)dataModel{
    ShieldPostVC *shieldPostVC = [[ShieldPostVC alloc] init];
    shieldPostVC.isFromThread = YES;
    shieldPostVC.dictPost = @{@"fid": @(detailModel.fid),
                              @"tid": @(detailModel.tid),
                              @"pid": @(dataModel.pid)};
    shieldPostVC.subject = detailModel.subject;
    shieldPostVC.isShield = (dataModel.status == 1 || dataModel.status == 3) ? YES : NO;
    [vc.navigationController pushViewController:shieldPostVC animated:YES];
}
